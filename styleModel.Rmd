...
A <- matrix(c(ws$count, ws$count^2, ws$count^3), nrow=length(ws$count))
...
Introducing whitespace gives a cost that we assume fits a cubic with 0 intercept, and constrained positive parameters of `r fit$x`.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
ws$sample <- basefit$coefficients[1] + ws$count * (fit$x[1] + basefit$coefficients[2]) + ws$count^2 * fit$x[2] + ws$count^3 * fit$x[3]
