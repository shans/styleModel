...
fit <- lm(ws$Document..updateStyle ~ 0 + I(ws$count^2))
ws$Document..updateStyle = ws$Document..updateStyle + basefit$coefficients[1] + ws$count * basefit$coefficients[2];
...
Introducing whitespace gives a cost that we assume fits a pure quadratic with a count^2 term of `r fit$coefficients[1]`.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
ws$sample <- basefit$coefficients[1] + ws$count * ws$count * fit$coefficients[1]
